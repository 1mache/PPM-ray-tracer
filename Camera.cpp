...
Camera::Camera(const Dimensions& screenSize,
			   const Vec3& position, 
			   const Vec3& lookat,
			   float FOV):
	m_screenSize(screenSize), m_lookfrom(position), m_lookat(lookat), m_FOV(FOV)
{
	float screenAspectRatio = float(m_screenSize.width) / m_screenSize.height;
	...
	m_pixelSize = Vec3(m_viewportWidth / m_screenSize.width,
					   m_viewportHeight / m_screenSize.height,
					   0.0f);
...
Vec3 Camera::screenToViewportPos(const Dimensions& pixelCords) const
{
	...
	Dimensions flippedCords = Dimensions( pixelCords.width , m_screenSize.height - pixelCords.height); 
	...
	Vec3 pixelOffset = Vec3((float(flippedCords.width) / (m_screenSize.width - 1)),
							(float(flippedCords.height) / (m_screenSize.height - 1)),
							0.0f);	
